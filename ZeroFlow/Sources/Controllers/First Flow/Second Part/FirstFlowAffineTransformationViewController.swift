//
//  FirstFlowAffineTransformations.swift
//  ZeroFlow
//
...
final class FirstFlowAffineTransformationViewController: UIViewController {
    ...
    private var mathQueue = DispatchQueue(label: "com.medium.math.affine", qos: .userInteractive)
    ...
    @IBAction func transformButtonPressed(_ sender: UIButton) {
        ...
        if isTransformApplied {
            drawInitialState()
            transformButton.setTitle("Affine transform", for: .normal)
            
            isTransformApplied = false
            
        } else {
            let affineDataInputVC: FirstFlowAfifneTransformInputViewController = .instantiate(from: .first)
            affineDataInputVC.delegate = self
            
            present(affineDataInputVC, animated: true)
        }
    }

}

extension FirstFlowAffineTransformationViewController: ParametersViewControllerDelegate {
...
extension FirstFlowAffineTransformationViewController: FirstFlowAfifneTransformInputViewControllerDelegate {
    ...
    func applyAffineTransformData(points: [CGPoint]) {
        ...
        mathQueue.async {
            self.shapeTransformer.configure(shape: self.gridShape)
            self.shapeTransformer.affineTransformation(basises: points)
            self.gridShape = self.shapeTransformer.build()

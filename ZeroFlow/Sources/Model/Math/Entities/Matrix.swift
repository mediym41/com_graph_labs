...
public struct Matrix<T: Number> {
    ...
    public init(rows: [[T]]) {
        assert(!rows.isEmpty, "Matrix has to have elements")
        
        self.rows = rows.count
        self.columns = rows[0].count
        self.size = Size(rows: self.rows, columns: columns)
        self.grid = rows.flatMap { $0 }
        
        rows.forEach { row in
            assert(row.count == self.columns, "Rows size are not equal")
        }
    }
    ...
    public func forEachRow(_ body: ([T]) throws -> Void) rethrows {
        for rowIndex in 0 ..< rows {
            try body(row(for: rowIndex))
        }
    }
    
    public func mapRows<M>(_ transform: ([T]) throws -> M) rethrows -> [M] {
        
        var result: [M] = []
        
        for rowIndex in 0 ..< rows {
            let item = try transform(row(for: rowIndex))
            result.append(item)
        }
...
extension Matrix: Multipliable {
    public static func *<T: Number>(lhs: Matrix<T>, rhs: Matrix<T>) -> Matrix<T> {
        return lhs.matrixMultiply(by: rhs)
    }
}
